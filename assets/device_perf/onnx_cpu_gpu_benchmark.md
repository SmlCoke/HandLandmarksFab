...
Eos-2.1 Palm Detector 与 v1 MobileNetV3-Large HCF 在 RTX 3090 上均显著快于 CPU，且人工 Gold 回放未发生分类、正式解码或采用门控的跨阈值变化，因此继续使用 `auto`（CUDA 优先、CPU fallback）。RTMPose 模型未更新，继续沿用上一轮人工 Gold 设备结论并固定 CPU。RTMPose/HCF batch 为 64；Palm 输入固定 batch 1。
...
| HCF | `models/hand_classifier/v1-mobilenet_v3_large/model.onnx` |
...
| HCF v1 MobileNetV3-Large | 64 | 207.4 | 4265.5 | 20.56× | GPU，CPU fallback |
...
| HCF v1 MobileNetV3-Large | 9,279 条 Gold ROI；logits 最大差 `0.009796`、概率最大差 `0.000623` | presence/handedness argmax、presence 0.5 与 handedness 0.8 均 0 次变化 |
