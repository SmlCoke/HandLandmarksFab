...
Palm Detector 与 Hand Classifier（HCF）在 GPU 上明显更快，且未改变本次回放的发布判断，默认使用 `auto`（CUDA 优先、不可用时回退 CPU）。RTMPose GPU 虽快约 27 倍，但对全部 9,868 条人工复核 Eval 回放的平均关键点误差比 CPU 高 `0.0145 px`，因此按精度优先原则固定使用 CPU。RTMPose/HCF 默认 batch 为 `64`；Palm 模型输入固定为 batch 1。
...
| Palm | Eos 1.0，`models/palm_detector/eos-1.0/model_opt.onnx` |
...
- 吞吐：固定随机种子生成同一 float32 张量，分别用 CPU/CUDA provider 预热 5 次；Palm/HCF 重复 80 次，RTMPose 重复 20 次，记录纯模型推理耗时中位数。动态模型比较 batch `1/8/32/64`，下表展示典型大量 ROI 场景的最快已测配置。
- Palm 一致性：在正式仓库中只读抽取 64 张真实 TIFF，比较四个原始输出以及按正式阈值完成 NMS 后的 detection/candidate。
...
|---|---:|---:|---:|---:|---|
| Palm Detector | 1（模型固定） | 124.5 | 578.3 | 4.65× | GPU，CPU fallback |
...
| 模型 | 回放结果 | 判定 |
|---|---|---|
| Palm | 真实图片 raw 最大绝对差约 `1.1e-6`；decoded detection/candidate 数量一致，几何/分数最大差约 `1.0e-6` | 未观察到发布级变化，采用 GPU |
