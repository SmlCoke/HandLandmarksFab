...
6. Treat supported capture distances as a Palm-model capability contract. When updating Eos or another Palm Detector, evaluate near/mid/far coverage and update the explicit supported-distance configuration; never inherit distance support from the previous model or delete raw capture sources as a substitute for a model capability gate.
