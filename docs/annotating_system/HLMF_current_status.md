...
原图可视化分支已接入 Train/Eval autolabel：`visualization.original_image_enabled` 默认为 `false`，单次可用 `ORIGINAL_VISUALIZATION=true|false` 覆盖。启用后使用 draft 的 `landmarks_image_px` 在来源全部原图上绘制关键点，输出到 `visualizations/original_image_landmarks/<variant>/`，并严格保留原图文件名和扩展名，便于逐图比较不同阈值变体。已有 draft 可通过 `make autolabel-visualize-original ...` 直接补生成。
