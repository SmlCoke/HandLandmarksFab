...
if (( PREFLIGHT_FAILED > 0 )); then
    echo "ERROR: Eval batch aborted before annotation because distance preflight failed" >&2
    exit 2
fi
if (( ${#SUPPORTED_SOURCE_DIRS[@]} == 0 )); then
    printf 'ERROR: Eval batch has no Palm-compatible sources: discovered=%d skipped=%d\n' \
...
SUCCESS=0
FAILED=0
printf 'Eval batch: dataset=%s variant=%s backend=%s discovered=%d supported=%d skipped=%d\n' \
...
printf 'Eval batch complete: discovered=%d supported=%d success=%d failed=%d skipped=%d logs=%s\n' \
